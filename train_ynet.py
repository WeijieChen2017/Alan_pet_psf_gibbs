...
def progress_eval(mri_input, mri_output, pet_input, model, loss_fn, epochs, img_num, save_name):
    ...
    for idx in range(img_num):
        ...
        img_mri_output = np.squeeze(mri_output[mri_output[0]//2, :, :, 0])
        img_mri_eval = np.squeeze(mri_eval[mri_eval.shape[0]//2, :, :, 0])
        img_pet_input = np.squeeze(pet_input[pet_input[0]//2, :, :, pet_input[3]//2])
        img_pet_eval = np.squeeze(pet_eval[pet_eval.shape[0]//2, :, :, 0])
